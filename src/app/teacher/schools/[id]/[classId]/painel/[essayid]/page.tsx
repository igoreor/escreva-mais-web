...
import { FiHome, FiBookOpen, FiUser, FiFileMinus, FiArrowLeft, FiMessageCircle } from "react-icons/fi";
...
export default function TemaDetalhesPage() {
  ...
  const tema = {
    ...
    textos: [
      {
        titulo: "TEXTO I",
        conteudo: "O trabalho de cuidado não remunerado e mal pago é essencial para nossas sociedades e para a economia. Ele inclui o trabalho de cuidar de crianças, idosos e pessoas com doenças e deficiências físicas e mentais, bem como o trabalho doméstico diário que inclui cozinhar, limpar, lavar, consertar coisas e buscar água e lenha. Se ninguém investisse tempo, esforços e recursos nessas tarefas diárias, economias inteiras ficariam estagnadas...",
      ...
      {
        titulo: "TEXTO III",
        conteudo: "A sociedade brasileira tem passado por inúmeras transformações sociais ao longo das últimas décadas. Entre elas as percepções sociais a respeito dos valores e das convenções de gênero e a forma como mulheres têm se inserido na sociedade.",
    ...
    <RouteGuard allowedRoles={['teacher']}>
      <div className="flex min-h-screen bg-gray-50">
        ...
        <main className="flex-1 lg:ml-[270px] p-6 lg:p-10">
         ...
         {/* Título */} <div className="bg-white rounded-lg shadow p-6"> <h1 className="text-2xl font-bold text-gray-900 mb-6">{tema.titulo}</h1> <h2 className="text-lg font-semibold text-blue-700 mb-4">Textos motivadores</h2> <div className="flex flex-col gap-6"> {tema.textos.map((t, idx) => ( <div key={idx} className="bg-gray-50 border rounded-lg p-4"> <h3 className="font-bold mb-2">{t.titulo}</h3> {t.conteudo && <p className="text-gray-700 mb-2 whitespace-pre-line">{t.conteudo}</p>} {t.imagem && ( <img src={t.imagem} alt={t.titulo} className="rounded-lg border mb-2 max-w-md" /> )} {t.fonte && <p className="text-sm text-gray-500">📖 {t.fonte}</p>} </div> ))} </div> </div>
              ...
              {entregas.map((entrega) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 text-sm font-bold rounded-lg border ${getNotaColor(entrega.nota)}`}>
